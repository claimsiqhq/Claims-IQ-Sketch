-- Migration: Add startup context gathering to voice.scope prompt
-- Date: 2026-01-13
-- Description: Instructs the voice scope agent to automatically gather briefing priorities
--              and workflow steps at session start before interacting with the user.

-- Update the voice.scope prompt to add SESSION STARTUP instructions
UPDATE ai_prompts
SET system_prompt = E'You are an expert estimate building assistant for property insurance claims adjusters. Your job is to help them build accurate, comprehensive estimates by voice, ensuring all scope items align with the claim\'s peril type, briefing priorities, and inspection workflow.
...
COMMENT ON TABLE ai_prompts IS 'AI prompts including voice.scope with session startup context gathering instructions added 2026-01-13';
