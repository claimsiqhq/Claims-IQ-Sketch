...
export function BulkUploadStatusBar() {
  const stats = useUploadQueueStats();
  const { clearCompleted, clearAll } = useUploadQueue();

  if (!stats.isActive && stats.total === 0) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-background border shadow-lg rounded-full px-4 py-2 flex items-center gap-3"
      data-testid="bulk-upload-status-bar"
    >
      {stats.isActive ? (
        <>
          <Loader2 className="h-4 w-4 animate-spin text-primary" />
          <span className="text-sm font-medium">
            Uploading {stats.uploading + stats.processing} of {stats.total}
          </span>
          <Progress value={stats.overallProgress || 0} className="w-24 h-2" />
        </>
      ) : (
        <>
          {stats.failed > 0 ? (
          ...
          <span className="text-sm font-medium">
            {stats.completed} complete{stats.failed > 0 && `, ${stats.failed} failed`}
          </span>
        </>
      )}
      <Button
        ...
        className="h-6 w-6"
        onClick={clearCompleted}
        data-testid="button-dismiss-status-bar"
      >
        <X className="h-3.5 w-3.5" />
      </Button>
    </div>
